...
`src-tauri/crates/tolaria-server` is an Axum binary crate that serves the vault to browser clients. It has no Tauri dependency and links `tolaria-core` directly for all vault/search logic. See [ADR-0161](./adr/0161-web-server-read-only-phase.md).
...
Phase 3 adds built-in auth in front of the Phase 2 read-only server. See [ADR-0162](./adr/0162-web-server-builtin-auth.md).
...
Phase 4 adds authenticated write commands over the Phase 3 auth layer. See [ADR-0163](./adr/0163-web-server-write-path-optimistic-concurrency.md).
...
The browser build runs the **unmodified desktop React app** against `tolaria-server`. Bridging the two required a small compatibility layer, documented in [ADR-0164](./adr/0164-web-client-compatibility-layer.md).
...
Phase 5 adds authenticated git commit/push/pull/status/conflict-resolution controls on top of the Phase 4 write path. See [ADR-0165](./adr/0165-web-server-git-sync-model.md).
...
- **Authorship**: `AppState::acting_user` resolves the session cookie to a `CommitIdentity` — author = committer = the acting user's own `git_name`/`git_email` (from `UsersDb`, stored since ADR-0162). Every commit the server makes (autogit or explicit) attributes fully to the acting user, so per-user attribution survives even though the server holds one shared push credential. `GIT_COMMITTER_*` env vars still feed pull-merge commits (which have no single acting user), configured from `TOLARIA_COMMITTER_NAME`/`TOLARIA_COMMITTER_EMAIL` at deploy time.
...
plus vault folder mutations. See [ADR-0166](./adr/0166-web-command-surface-completion.md).
