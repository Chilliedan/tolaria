...
`src-tauri/crates/tolaria-server` is an Axum binary crate that serves the vault to browser clients. It has no Tauri dependency and links `tolaria-core` directly for all vault/search logic. See [ADR-0901](./adr/0901-web-server-read-only-phase.md).
...
Phase 3 adds built-in auth in front of the Phase 2 read-only server. See [ADR-0902](./adr/0902-web-server-builtin-auth.md).
...
Phase 4 adds authenticated write commands over the Phase 3 auth layer. See [ADR-0903](./adr/0903-web-server-write-path-optimistic-concurrency.md).
...
The browser build runs the **unmodified desktop React app** against `tolaria-server`. Bridging the two required a small compatibility layer, documented in [ADR-0904](./adr/0904-web-client-compatibility-layer.md).
...
- **Vault image assets** (`asset_route.rs`): the web `convertFileSrc` maps a vault file path to `/api/asset/<encodeURIComponent(path)>`, mirroring Tauri's `asset://localhost/<encoded>` shape. `'/api/asset/'` is registered in `vaultAttachments.ts`'s asset-URL prefixes, so editor round-trips back to portable `attachments/...` paths are unchanged. The authenticated `GET /api/asset/*path` route serves only image types that canonicalize to a file inside `vault_root` (404 otherwise), with `nosniff`, `Cache-Control: private`, and a sandbox CSP for SVG. See [ADR-0907](./adr/0907-web-server-vault-image-assets.md).
...
Phase 5 adds authenticated git commit/push/pull/status/conflict-resolution controls on top of the Phase 4 write path. See [ADR-0905](./adr/0905-web-server-git-sync-model.md).
...
plus vault folder mutations. See [ADR-0906](./adr/0906-web-command-surface-completion.md).
