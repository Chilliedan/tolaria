...
/// App-configured git program selection (custom git binary path, WSL provider).
/// tolaria-core has no `Settings` type of its own; the host app registers a
/// provider function once at startup via [`set_git_program_config_provider`]
/// that reads its own settings store. Callers with nothing registered (e.g.
/// tolaria-server) transparently fall back to plain `git` on PATH.
#[derive(Debug, Clone, Default)]
pub struct GitProgramConfig {
    pub git_path: Option<String>,
    pub git_provider: Option<String>,
    pub git_wsl_distro: Option<String>,
}

type GitProgramConfigProvider = fn() -> Option<GitProgramConfig>;

static GIT_PROGRAM_CONFIG_PROVIDER: OnceLock<GitProgramConfigProvider> = OnceLock::new();

/// Register the host app's settings lookup. Call once at startup; later
/// calls are ignored (first registration wins).
pub fn set_git_program_config_provider(provider: GitProgramConfigProvider) {
    let _ = GIT_PROGRAM_CONFIG_PROVIDER.set(provider);
}
...
pub(crate) fn git_path_argument(path: &str) -> Result<String, String> {
    provider::selected_git_path_argument(path, git_program_config().as_ref())
}
...
fn detect_git_launch_config() -> GitLaunchConfig {
    ...
    let config = git_program_config();
    if let provider::GitProviderSelection::Wsl { distro } =
        provider::GitProviderSelection::from_config(config.as_ref())
    {
...
fn configured_git_path() -> Option<PathBuf> {
    git_program_config()
        .and_then(|config| config.git_path)
        .map(PathBuf::from)
