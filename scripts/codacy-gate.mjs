...
const requestedBase = resolveBaseRef({
  branch,
  envBase: process.env.CODACY_BASE_REF,
  upstreamRef: gitOrNull(['rev-parse', '--abbrev-ref', '@{push}'])
    ?? gitOrNull(['rev-parse', '--abbrev-ref', '@{upstream}']),
})
